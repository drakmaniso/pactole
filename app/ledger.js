...
export async function open(name) {
  return new Promise((resolve, reject) => {
    ...
    req.onerror = event =>
      reject(
        new Error(
          `ledger.open: failed to open database "${name}": ${event.target.error}`,
        ),
      )
    req.onblocked = event =>
      reject(new Error(`ledger.open: database "${name}" is blocked`))
    ...
    req.onsuccess = event => {
      ...
      tr.onerror = event =>
        reject(new Error(`ledger.open transaction: ${event.target.error}`))
      tr.oncomplete = () => resolve(accounts)

      const os1 = tr.objectStore('accounts')
      const req1 = os1.getAll()
      req1.onerror = event =>
        reject(new Error(`ledger.open accounts initialization: ${req1.error}`))
      req1.onsuccess = () => {
...
export function getAccounts() {
  return new Promise((resolve, reject) => {
    ...
    const req = os.getAll()
    req.onerror = event =>
      reject(new Error(`ledger.open accounts initialization: ${req.error}`))
    req.onsuccess = () => {
